...
ce mod\`ele est que l'ajout de nouvelles entit\'es est assez fastidieux : à
chaque nouvel objet il faut une méthode pour le créer. Dans les constructeurs
...
\subsection*{ProblÃ¨mes rencontrÃ©s}

Nous avons bien sur rencontrÃ© plusieurs points difficiles.

\subsubsection*{Gestion des niveaux}

Nous avons mis en place plusieurs niveaux ainsi qu'un changement de ceux-ci, 
ce qui s'avÃ©ra \^etre une t\^ache ardu. En effet, nous pouvons passer au niveau 
suivant sans problÃ¨me mais choisir au hasard un niveau et le lancÃ© est malheureusement
impossible. Nous avons voulu mettre en place un systÃ¨me utilisant ceci via le rechargement
de sauvegarde. Avec notre implÃ©mentation, quand nous rechargeons un niveau, le niveau se 
charge mais celui-ci ne dÃ©marre pas. Nous avons tentÃ© de diverse faÃ§on de faire en sorte que
cela fonctionne sans rÃ©sultat. En fait, le plus compliquÃ© vient de l'utilisation des threads
qui doivent Ãªtre interrompus avant de passer au niveau suivant et le niveau suivant doit Ãªtre
lancÃ©. Mais pour des raisons inconnus, les threads ne se lancent pas.

\subsubsection*{XML}

En thÃ©orie, il est possible de crÃ©er des niveaux au format XML grÃ¢ce au Monteur LevelReader, malheureusement
nous avons eu des difficultÃ©s Ã  mettre en place ce lecteur de XML. Nous n'avons pas rÃ©ussi surement 
Ã  cause du fait que nous n'avions jamais utilisÃ© l'API correspondante auparavant, et les diffÃ©rents 
tutoriaux prÃ©sent sur internet n'ont pas suffit Ã  nous permettre de rÃ©ussir.

\subsubsection*{Le framework de base}

Le fait d'Ãªtre confrontÃ© Ã  un framework dont nous ne pouvions rien toucher, fut un challenge intÃ©ressant 
mais cependant parfois compliquÃ©. Il nous est arrivÃ© Ã  plusieurs reprise de vouloir rÃ©cupÃ©rer une chose
d'une classe du framework, alors que cette chose ne nous Ã©tait pas accessible ou encore modifier une variable
privÃ©e inaccessible de l'extÃ©rieur. Nous avons avoir parfois Ã©tait tentÃ© par la recopie de code (ce que nous 
pouvons apercevoir avec la classe GameZeldaAWTImpl) mais il fallait le plus souvent trouvÃ© des astuces. Ces astuces 
Ã©taient souvent des modÃ¨les de conceptions, par exemple nous avons utilisÃ© un dÃ©corateur pour permettre d'Ã©tendre 
les fonctionnalitÃ©s d'une interface qui ne nous permettait pas de rÃ©cupÃ©rer une variable pourtant essentielle 
sans laquelle le jeu aurait paru Ã©trange aux yeux des divers joueurs.
