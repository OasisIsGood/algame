...
Notre version s'inspire d'une
application Facebook\footnote{\url{http://apps.facebook.com/zeldarpg/index.php}}
où l'on peut guider dans une forêt le personnage Link. Il peut utiliser son épée
avec la touche ``Espace'', comme notre version. \\
...
secrets que l'on peut chercher si l'on veut. Qu'il est agréable étant joueur de
trouver ce genre de code!

\section{Conception}

\subsection{Pacquetages}
Le paquetage ``zelda'' comprend plusieurs paquetages.\\
Le paquetage ``base'' permet de gérer l'interaction utilisateur, sons, gestion
de mouvements de personnages.\\ Le paquetage ``rule'' permet de gérer les
collisions ou blocages entre entités du jeu.\\ Le paquetage ``level'' permet de
gérer la mise en place des niveaux du jeu, de la lecture de fichier pour les niveaux, à la création de niveaux.\\ Le paquetage ``game''\\ Le paquetage ``observer'' permet de gérer tout les
observateurs du jeu.\\ Le paquetage ``entity'' possède toutes les classes de toutes les entités
présentes dans le jeu : personnages et décors. Dans les personnages, un
paquetage est réservé aux états de Link.\\
...
DIAGRAMME DE L'ETAT
\begin{center}
 %\includegraphics[width=9cm,height=9cm]{images/linkEtats.eps}
\end{center}
